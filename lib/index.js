...
$(function() {
  ...
  $('#foods').on('click', '#update', function(data){
    ...
    button.on('click', function(){
      ...
      $(parentArray[0]).replaceWith(`<td class="updater${parentId}" id='update'>${parentArray[0].value}</td>`)
      $(parentArray[1]).replaceWith(`<td class="updater${parentId}" id='update'>${parentArray[1].value}</td>`)
